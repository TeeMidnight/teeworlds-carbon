...
IGameControllerVanilla::IGameControllerVanilla(CGameContext *pGameServer) :
...
bool IGameControllerVanilla::GetPlayersReadyState(int WithoutID)
{
...
void IGameControllerVanilla::SetPlayersReadyState(bool ReadyState)
{
	for(int i = 0; i < MAX_CLIENTS; ++i)
	{
...
// balancing
bool IGameControllerVanilla::CanBeMovedOnBalance(int ClientID) const
{
	return true;
}

void IGameControllerVanilla::CheckTeamBalance()
{
...
void IGameControllerVanilla::DoTeamBalance()
{
...
// event
int IGameControllerVanilla::OnCharacterDeath(CCharacter *pVictim, CPlayer *pKiller, int Weapon)
{
	// do scoreing
	if(!pKiller || Weapon == WEAPON_GAME)
		return 0;
	if(pKiller == pVictim->GetPlayer())
		m_aPlayerScore[pKiller->GetCID()]--; // suicide or world
	else
	{
		if(IsTeamplay() && pVictim->GetPlayer()->GetTeam() == pKiller->GetTeam())
			m_aPlayerScore[pKiller->GetCID()]--; // teamkill
		else
			m_aPlayerScore[pKiller->GetCID()]++; // normal kill
	}
...
void IGameControllerVanilla::OnFlagReturn(CFlag *pFlag)
{
}

void IGameControllerVanilla::OnPlayerConnect(CPlayer *pPlayer)
{
...
void IGameControllerVanilla::OnPlayerDisconnect(CPlayer *pPlayer)
{
    ...
    SWorldStatus *pStatus = GetWorldStatus(pPlayer->GameWorld());
    if(!pStatus)
        return;

	if(pPlayer->GetTeam() != TEAM_SPECTATORS)
	{
		--pStatus->m_aTeamscore[pPlayer->GetTeam()];
		pStatus->m_UnbalancedTick = TBALANCE_CHECK;
	}

	CheckReadyStates(pPlayer->GameWorld(), ClientID);
}

void IGameControllerVanilla::OnPlayerInfoChange(CPlayer *pPlayer)
{
}

void IGameControllerVanilla::OnPlayerReadyChange(CPlayer *pPlayer)
{
	if(Config()->m_SvPlayerReadyMode && pPlayer->GetTeam() != TEAM_SPECTATORS && !pPlayer->m_DeadSpecMode)
	{
		...
		{
			SetGameState(pPlayer->GameWorld(), IGS_GAME_PAUSED, TIMER_INFINITE); // one player isn't ready -> pause the game
			GameServer()->SendGameMsg(GAMEMSG_GAME_PAUSED, pPlayer->GetCID(), -1);
		}

		CheckReadyStates(pPlayer->GameWorld());
	}
}

// to be called when a player changes state, spectates or disconnects
void IGameControllerVanilla::CheckReadyStates(CGameWorld *pGameWorld, int WithoutID)
{
	if(Config()->m_SvPlayerReadyMode)
	{
		...
		case IGS_WARMUP_USER:
			...
			if(GetPlayersReadyState(WithoutID))
				SetGameState(pGameWorld, IGS_WARMUP_USER, 0);
			break;
		case IGS_GAME_PAUSED:
			// all players are ready -> unpause the game
			if(GetPlayersReadyState(WithoutID))
				SetGameState(pGameWorld, IGS_GAME_PAUSED, 0);
			break;
...
void IGameControllerVanilla::OnReset()
{
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
...
// game
bool IGameControllerVanilla::DoWincheckMatch()
{
	if(IsTeamplay())
	{
...
void IGameControllerVanilla::ResetGame(CGameWorld *pGameWorld)
{
	// reset the game
	pGameWorld->m_ResetRequested = true;
...
void IGameControllerVanilla::SetGameState(CGameWorld *pGameWorld, EGameState GameState, int Timer)
{
	// change game state
	switch(GameState)
	{
	...
	case IGS_GAME_PAUSED:
		...
		if(m_GameState == IGS_GAME_RUNNING || m_GameState == IGS_GAME_PAUSED || m_GameState == IGS_WARMUP_GAME)
		{
			...
			else
			{
				...
				SetGameState(pGameWorld, IGS_START_COUNTDOWN);
			}
		}
		break;
	case IGS_END_ROUND:
...
void IGameControllerVanilla::StartMatch()
{
	ResetGame();
...
void IGameControllerVanilla::StartRound()
{
	ResetGame();
...
void IGameControllerVanilla::SwapTeamscore()
{
	if(!IsTeamplay())
		return;
...
// general
void IGameControllerVanilla::Snap(int SnappingClient)
{
...
void IGameControllerVanilla::Tick()
{
	// handle game states
	...
	// win check
	if((m_GameState == IGS_GAME_RUNNING || m_GameState == IGS_GAME_PAUSED) && !pGameWorld->m_ResetRequested)
	{
		if(m_GameFlags&GAMEFLAG_SURVIVAL)
			DoWincheckRound();
		else
...
// info
void IGameControllerVanilla::CheckGameInfo()
{
...
bool IGameControllerVanilla::IsFriendlyFire(int ClientID1, int ClientID2) const
{
...
bool IGameControllerVanilla::IsFriendlyTeamFire(int Team1, int Team2) const
{
	return IsTeamplay() && !Config()->m_SvTeamdamage && Team1 == Team2;
}

bool IGameControllerVanilla::IsPlayerReadyMode() const
{
...
bool IGameControllerVanilla::IsTeamChangeAllowed() const
{
	return !pGameWorld->m_Paused || (m_GameState == IGS_START_COUNTDOWN && m_GameStartTick == Server()->Tick());
}

void IGameControllerVanilla::SendGameInfo(int ClientID)
{
...
bool IGameControllerVanilla::GetStartRespawnState() const
{
	if(m_GameFlags&GAMEFLAG_SURVIVAL)
	{
...
// team
bool IGameControllerVanilla::CanChangeTeam(CPlayer *pPlayer, int JoinTeam) const
{
...
bool IGameControllerVanilla::CanJoinTeam(int Team, int NotThisID) const
{
...
int IGameControllerVanilla::ClampTeam(int Team) const
{
	if(Team < TEAM_RED)
...
void IGameControllerVanilla::DoTeamChange(CPlayer *pPlayer, int Team, bool DoChatMsg)
{
	Team = ClampTeam(Team);
...
int IGameControllerVanilla::GetStartTeam()
{
...
void IGameControllerVanilla::InitWorldConfig(CWorldConfig *pConfig)
{
	IGameController::InitWorldConfig(pConfig);
}
